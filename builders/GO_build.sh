...
#								--build-arg LOG_LEVEL="${log_level}"				\
	...
	; then
		...
		printf "\n\n\t\t"
		printf "TARGET_DOMAIN=%s TARGET_ALIAS=%s TARGET_IMAGE_TAG=%s LOG_LEVEL=%s SERVICE_NAME=%s  docker-compose build %s"	"${target_domain}" "${target_alias}" "${target_image_tag}" "${log_level}" "${service_name}" "${service_name}"
		printf "\n\n\n...quitting\n\n%s" "${reset}"
		exit 4
	fi
