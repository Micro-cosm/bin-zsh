checksigned() {
	...
	dig "${OPTS}" NS "${ZONE}" @publicdns.goog | {
		...
		while read -r DOMAIN TYPE NS; do # Check each delegated name server
			...
			if dig +cd +dnssec +norecurse DNSKEY "${ZONE}" "@${NS}" | grep -E 'RRSIG[[:space:]]+DNSKEY' > /dev/null; then
		...
		if "$NO_NS"; then
			...
			MINTTL=$(dig +cd SOA "${ZONE}" @publicdns.goog | awk '/^[^;]/ && $4=="SOA" { print $11 }')
